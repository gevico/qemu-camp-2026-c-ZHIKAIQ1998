 ...
 */
typedef struct {
    uint16_t version_be;   /* 版本字段 (大端序) */
    uint16_t length_be;    /* 长度字段 (大端序) */
    uint8_t  flags_raw;    /* 标志字段 */
} proto_header_raw_t;
...
int main(void) {
    ...
    uint8_t version_low = (uint8_t)(version_host & 0xFF);
    unsigned major = (version_low >> 4) & 0xF;
    unsigned minor = version_low & 0xF;
    ...
    view.ver_major = major;
    view.ver_minor = minor;
    view.length = length;
    view.reserved = (raw.flags_raw >> 5) & 0x7;
    view.flags = flags;
